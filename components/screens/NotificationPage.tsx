...
const NotificationPage = () => {
    ...
    async function fetchOrders(){
        ...
        await api.get(`${API_URL}/order/`).then((res)=> {
    ...
    <ScrollView>
      {orders.length === 0 && <ActivityIndicator size="large" color="black" />}
        ...
        <View key={order._id} style={{ padding: 20, backgroundColor: 'white' ,margin: 20}}>
          {order.items.map((item) => (
            <View key={item._id} style={{ flexDirection: 'row', marginBottom: 20 }}>
              ...
              <View style={{ flex: 1 }}>
                ...
                <ArtFeastText text={`Price: ${item.postId.price}`}  style={{fontSize:17}}/>
          ...
          <ArtFeastText text={`Artist Name: ${order.userId.fullName}`}  style={{fontSize:17}}/>
